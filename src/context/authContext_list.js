...
export const AuthProviderList = ({ children }) => {
  ...
  const predefinedTasks = [
    {
      title: 'Beber quantia ideal de água',
    ...
    {
        title: 'Treinar grupo muscular do dia',
        description: 'Melhorar a qualidade de vida.',
        flag: 'Exercício',
        color: '#EB5757', 
        isPredefined: true
    },
    {
        title: 'Comer alimentos nutritivos',
    ...
    {
        title: 'Praticar atividade de cardio',
        description: 'Melhore seu nível de energia.',
        flag: 'Atividade',
        color: '#F2C94C',
        isPredefined: true
    },
    {
      ...
      description: 'Priorize o sono de qualidade.',
      flag: 'Recuperação',
      color: '#9B51E0',
      isPredefined: true
    },
  ...
  const toggleTaskCompletion = async (task) => {
    const updatedTasks = tasks.map(t => 
      t.item === task.item ? { ...t, completed: !t.completed } : t
    );
        ...
        <View style={styles.optionsModalContent}>
          {selectedTask && (
            <>
              ...
              >
                <Text style={styles.modalOptionText}>{selectedTask.completed ? 'Cancelar Conclusão' : 'Concluir Atividade'}</Text>
              </TouchableOpacity>

              <TouchableOpacity 
                style={[styles.modalOption, styles.deleteOption]}
                onPress={() => {
                handleDeleteTask(selectedTask); // Exclui direto
                optionsModalRef.current?.close();
                }}
              >
...
// ESTILOS ATUALIZADOS (COM NOMES ESPECÍFICOS)
const styles = StyleSheet.create({
  ...
  // Estilos do MODAL DE ADICIONAR TAREFA (antigos com prefixo)
  addTaskModalContent: {
    padding: 25,
    paddingBottom: 40,
    backgroundColor: themes.colors.primary
